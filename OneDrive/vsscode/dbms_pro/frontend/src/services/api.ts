...
export const api = {
  ...
  // Designers
  getDesigners: () => fetcher<Designer[]>('/designers'),
  createDesigner: (data: Omit<Designer, 'id' | 'created_at'>) => fetcher<Designer[]>('/designers', { method: 'POST', body: JSON.stringify(data) }),
  updateDesigner: (id: number, data: Partial<Designer>) => fetcher<Designer[]>(`/designers/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
  deleteDesigner: (id: number) => fetcher<Designer[]>(`/designers/${id}`, { method: 'DELETE' }),
