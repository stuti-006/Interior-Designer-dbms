...
export default function Reports() {
  ...
  const budgetData = projects.map(p => ({
    ...
    budget: p.budget,
    paid: payments.filter(pay => (pay.projectId === p.id || pay.project_id === p.id) && pay.status === "paid").reduce((s, pay) => s + pay.amount, 0),
  }));

  // Designer workload
  const designerData = designers.map(d => ({
    name: d.name.split(" ")[0],
    projects: projects.filter(p => p.designerId === d.id || p.designer_id === d.id).length,
  }));
